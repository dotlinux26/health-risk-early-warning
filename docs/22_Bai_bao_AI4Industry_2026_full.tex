...
\begin{center}
    ...
    \textbf{Trạng thái:} BẢN NHÁP — Kết quả MIMIC-IV temporal validation đã bổ sung (2026-09-11)
\end{center}
...
\textbf{Kiểm định temporally trên MIMIC-IV v3.1} ($n=546.028$, split shifted years $\le$2115 / $\ge$2116): Logistic Regression AUC 0.752 ($\Delta$AUC $-0.010$), LightGBM AUC 0.751 ($\Delta$AUC $-0.033$). LR ổn định hơn trên dữ liệu ICU/ED Mỹ; capture rate top quintile $\sim$53--54\%. Kết quả khẳng định mô hình comorbidity-based (từ ICD-10) là driver chính do missingness vitals $\sim$60\%.
...
\subsection{Dữ liệu MIMIC-IV — External Validation}

\begin{itemize}
    \item \textbf{Nguồn:} MIMIC-IV Clinical Database v3.1 (Credentialed Access, DUA đã ký).
    \item \textbf{Kích thước:} 546.028 admissions (364.627 patients), shifted years 2105--2214.
    \item \textbf{Files core sử dụng:} patients, admissions, diagnoses\_icd, omr ($\sim$108 MB nén). \textit{Lưu ý: labevents/chartevents không có trong core files.}
    \item \textbf{Đặc trưng (từ OMR + ICD-10):} systolic\_bp, diastolic\_bp (parse từ ``Blood Pressure''), bmi, weight, height, egfr + 17 comorbidity flags (Charlson/Deyo từ ICD-10).
    \item \textbf{Missingness:} vitals/anthropometrics $\sim$60\% (OMR là outpatient, không phải inpatient), comorbidities 0\%.
    \item \textbf{Nhãn:} mortality\_30d (tử vong 30 ngày), in\_hospital\_mortality.
    \item \textbf{Temporal split:} Train shifted years $\le$2115 ($n=22.552$), Test $\ge$2116 ($n=523.476$).
...
\subsection{Kiểm định temporally trên MIMIC-IV}

\begin{table}[H]
\centering
\caption{Kết quả kiểm định temporally trên MIMIC-IV}
\label{tab:mimiciv_temporal}
\begin{tabular}{lcc}
\toprule
Chỉ số & LR & LightGBM \\
\midrule
AUC temporal test (shifted $\ge$2116) & \textbf{0.752} & 0.751 \\
AUC random test (đối chứng) & 0.761 & 0.784 \\
$\Delta$ AUC (temporal $-$ random) & \textbf{$-0.010$} & $-0.033$ \\
Capture rate top quintile (mortality\_30d) & 52.8\% & 54.3\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item LR ổn định hơn: $\Delta$AUC nhỏ ($-0.010$), phù hợp baseline triển khai.
    \item LightGBM overfitting dữ liệu tĩnh: $\Delta$AUC $-0.033$ lớn hơn.
    \item AUC thấp hơn NHANES-LMF (0.75 vs 0.82) do: (1) missingness vitals $\sim$60\%, (2) comorbidity flags là driver chính, (3) outcome 30d mortality trong ICU có pattern khác tử vong dân cư.
    \item Capture rate $\sim$53--54\% top quintile: mô hình phát hiện được $>$50\% ca tử vong 30d trong nhóm rủi ro cao nhất.
\end{itemize}

Tham chiếu: \texttt{experiments/EXP-TEMPORAL-MIMICIV/summary.json}, \texttt{scripts/run\_temporal\_mimiciv.py}.
...
\begin{enumerate}
    \item \textbf{Missingness vitals $\sim$60\% trên MIMIC-IV}: OMR chỉ ghi outpatient measurements, thiếu inpatient vitals/labs (cần labevents/chartevents) $\to$ AUC giảm so với NHANES-LMF.
    \item Outcome = tử vong (all-cause), không phải disease onset -- \cite{shmatko2025} đo được onset vì dùng UK Biobank dọc.
    \item FIB-4, calcium, eGFR-based CKD staging chưa triển khai (thiếu guideline PDF / OMR không có creatinine).
    \item Chưa có kết quả trên bệnh nhân Việt Nam thật (pilot cần thiết).
    \item Chưa có chuyên gia lâm sàng review 9 luật (governance do tên tự đặt).
    \item KNHANES yêu cầu KDCA RDC proposal -- \cite{shen2025} thừa nhận rào cản privacy/data access.
\end{enumerate}
...
\begin{itemize}
    \item Hệ thống đã được thiết kế, triển khai, kiểm định trên dữ liệu công khai (NHANES 3 chu kỳ, NHANES-LMF, MIMIC-IV v3.1).
    \item \textbf{NHANES-LMF} (dân cư): LR AUC 0.821, C-index 0.822, lead time 9 tháng -- tốt cho screening dân cư.
    \item \textbf{MIMIC-IV} (ICU/ED Mỹ): LR AUC 0.752, $\Delta$AUC $-0.010$, capture rate 53\% -- ổn định nhưng AUC thấp hơn do missingness vitals $\sim$60\%, comorbidity-driven.
    \item \cite{guo2024} Xác nhận GBM là baseline mạnh với chi phí thấp.
    \item \cite{swinckels2024} Trả lời cho khoảng trống external validation (chỉ 10\% nghiên cứu có) -- đã validate trên 2 dataset temporal độc lập.
    ...
    \item Hướng tiếp theo: KNHANES, pilot study nhỏ, tích hợp labevents/chartevents khi có full MIMIC-IV access.
\end{itemize}
...
\begin{itemize}
    \item[] $\blacksquare$ Bổ sung kết quả MIMIC-IV temporal validation (đã hoàn tất 2026-09-11)
...
\begin{center}
    \textit{Cập nhật lần cuối: 2026-09-11 (MIMIC-IV temporal validation added)}
\end{center}
